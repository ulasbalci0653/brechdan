...
export default function MenuSection() {
  const [selectedCategory, setSelectedCategory] = useState<MenuCategoryKey>("retro-isiriklar");
  ...
  const activeCategoryInfo = useMemo(() => {
    return MENU_CATEGORIES.find((c) => c.key === selectedCategory) || MENU_CATEGORIES[0];
  }, [selectedCategory]);
  ...
  // Filtered drinks if searching under drinks
  const filteredDrinks = useMemo(() => {
    if (selectedCategory !== "icecekler") return [];
    if (!searchQuery.trim()) return DRINKS;
    const query = searchQuery.toLowerCase();
    return DRINKS.filter(
      (d) =>
        d.name.toLowerCase().includes(query) ||
        (d.variants && d.variants.toLowerCase().includes(query))
    );
  }, [selectedCategory, searchQuery]);

  // Filtered extras if searching under extras
  const filteredSandwichExtras = useMemo(() => {
    if (selectedCategory !== "ekstralar") return [];
    if (!searchQuery.trim()) return SANDWICH_EXTRAS;
    const query = searchQuery.toLowerCase();
    return SANDWICH_EXTRAS.filter((e) => e.name.toLowerCase().includes(query));
  }, [selectedCategory, searchQuery]);

  const filteredBreakfastExtras = useMemo(() => {
    if (selectedCategory !== "ekstralar") return [];
    if (!searchQuery.trim()) return BREAKFAST_EXTRAS;
    const query = searchQuery.toLowerCase();
    return BREAKFAST_EXTRAS.filter((e) => e.name.toLowerCase().includes(query));
  }, [selectedCategory, searchQuery]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-10">
          ...
          <p className="text-sm sm:text-base text-[#5C675E] leading-relaxed">
            İncelemek istediğiniz kategoriyi seçerek ürünleri ve güncel fiyatları görüntüleyebilirsiniz.
          </p>
        </div>

        {/* Category Tabs (Horizontally scrollable on mobile) */}
        <div className="max-w-6xl mx-auto mb-10 overflow-x-auto pb-3 scrollbar-none">
            ...
            {MENU_CATEGORIES.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 sm:px-5 py-2.5 rounded-full text-xs sm:text-sm font-bold transition-all whitespace-nowrap cursor-pointer ${
                    isSelected
                      ? "bg-[#1B4332] text-[#FAF7F0] shadow-md scale-105"
                      : "bg-[#FFFFFF] text-[#3D4741] border border-[#E5DEC9] hover:bg-[#E8F0EA] hover:text-[#1B4332]"
                  }`}
                >
                  {cat.name}
                </button>
        ...
        <div className="max-w-4xl mx-auto mb-10 space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder={`${activeCategoryInfo.name} içinde ara... (Örn: Tiftik, Avokado, Omlet, vb.)`}
          ...
          {/* Quick Filters (Shown for food categories) */}
          {selectedCategory !== "ekstralar" && selectedCategory !== "icecekler" && (
            <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
              ...
              <span className="text-xs text-[#5C675E] font-medium">
                {currentCategoryItems.length} ürün listeleniyor
              </span>
            </div>
          )}
        </div>

        {/* Selected Category Content Presentation */}
        <div className="max-w-6xl mx-auto">
          
          {/* Standard Food Categories (Retro Isırıklar, Yeşil Isırıklar, Tavadan Klasikler, Kabaran Klasikler, Kahvaltı Tabakları) */}
          {selectedCategory !== "ekstralar" && selectedCategory !== "icecekler" && (
            <div className="space-y-8 animate-fade-in">
              {/* Category Header */}
              <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-2 pb-4 border-b-2 border-[#1B4332]">
                <div>
                  <div className="flex items-center gap-2">
                  ...
                  <p className="text-xs sm:text-sm text-[#5C675E] mt-1">
                    {activeCategoryInfo.subtitle}
                  </p>
                </div>

                {selectedCategory.includes("isiriklar") && (
                  <span className="text-[11px] font-semibold text-[#E05A2B] bg-[#E05A2B]/10 px-3.5 py-1.5 rounded-full shrink-0 border border-[#E05A2B]/20">
                    Ekşi Mayalı Ciabatta ile
                  </span>
                )}
                {selectedCategory === "kahvalti-tabaklari" && (
                  <span className="text-[11px] font-semibold text-[#1B4332] bg-[#E8F0EA] px-3.5 py-1.5 rounded-full shrink-0 border border-[#B7CDB9]">
                    Kupa Çay Dahil
                  </span>
                )}
              </div>
              ...
              {currentCategoryItems.length === 0 && (
                <div className="text-center py-12 bg-[#FFFFFF] rounded-3xl border border-[#E5DEC9] p-8">
                  ...
                  <p className="text-xs text-[#5C675E] mt-1">
                    Arama terimini temizleyerek veya filtreleri kaldırarak tekrar deneyebilirsiniz.
                  </p>
                  <button
                    onClick={() => {
                      setSearchQuery("");
          ...
          {/* EKSTRALAR Category Selected */}
          {selectedCategory === "ekstralar" && (
            <div className="space-y-8 animate-fade-in">
              <div className="pb-4 border-b-2 border-[#1B4332]">
                <div className="flex items-center gap-2">
                  <span className="text-[#E05A2B] text-lg font-bold">✦</span>
                  <h3 className="font-serif text-2xl sm:text-3xl font-bold text-[#1B4332] uppercase tracking-wide">
                    EKSTRALAR
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Sandviç Ekstraları */}
                <div className="bg-[#E05A2B]/10 rounded-3xl p-6 sm:p-8 border border-[#E05A2B]/30 shadow-sm">
                  <div className="flex items-center justify-between pb-4 mb-4 border-b border-[#E05A2B]/20">
                    ...
                    <span className="text-xs font-bold px-2.5 py-1 bg-[#E05A2B] text-[#FAF7F0] rounded-full">
                      {filteredSandwichExtras.length} Çeşit
                    </span>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
                    {filteredSandwichExtras.map((extra) => (
                      <div
                        key={extra.name}
                ...
                {/* Kahvaltı Ekstraları */}
                <div className="bg-[#E8F0EA] rounded-3xl p-6 sm:p-8 border border-[#B7CDB9] shadow-sm">
                  <div className="flex items-center justify-between pb-4 mb-4 border-b border-[#B7CDB9]">
                    ...
                    <span className="text-xs font-bold px-2.5 py-1 bg-[#1B4332] text-[#FAF7F0] rounded-full">
                      {filteredBreakfastExtras.length} Çeşit
                    </span>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
                    {filteredBreakfastExtras.map((extra) => (
                      <div
                        key={extra.name}
